...
const Folder:FC<FolderProps> = () => {
  ...
  return (
    <Container maxW="container.md" py="24px">
      {folderInfo && (
        <>
          <Flex pb="20px" justifyContent="space-between" alignItems="center">
            <Text fontSize='xl' fontWeight="bold">{folderInfo.team?.name ?? 'unknown'} / {folderInfo.name}</Text>
            <IconButton borderRadius="50%" size="sm" aria-label="创建项目" icon={<Icon as={FiPlus} />} onClick={handleModelOpen}/>
          </Flex>
      ...
      <ProjectCreationModal folderId={numFolderId} isOpen={isOpen} onClose={handleModelClose}></ProjectCreationModal>
    </Container>
  )
}
