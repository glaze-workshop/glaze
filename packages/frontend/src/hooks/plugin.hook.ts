...
export function usePlugins(ownerTeamId?: number) {
  const pluginsQuery = useQuery([PluginApi.FULL_PLUGIN_PATH, ownerTeamId], () =>
    PluginApi.getPlugins({ ownerTeamId })
  )
...
export function usePluginById(pluginId: string) {
  const pluginQuery = useQuery(PluginApi.FULL_PLUGIN_PATH_WITH_ID_TO_PATH(pluginId), () =>
    PluginApi.getPluginById(pluginId)
  )
