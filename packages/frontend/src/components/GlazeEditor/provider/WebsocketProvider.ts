...
interface MessageHandler {
  (encoder: encoding.Encoder, decoder: decoding.Decoder, provider: WebSocketProvider, emitSynced: boolean, messageType: number): void
}

const messageHandlers: MessageHandler[] = []

messageHandlers[EditorMessageEvent.SYNC] = (encoder, decoder, provider, emitSynced, messageType) => {
...
messageHandlers[EditorMessageEvent.AWARENESS] = (encoder, decoder, provider, emitSynced, messageType) => {
...
const readMessage = (provider: WebSocketProvider, buf: Uint8Array, emitSynced: boolean) => {
  ...
  const encoder = provider.createEncoder()
  ...
  if (provider.shouldConnect && provider.ws === null) {
    ...
    websocket.onopen = () => {
      ...
      if (provider.awareness.getLocalState() !== null) {
        const encoderAwarenessState = provider.createEncoder()
